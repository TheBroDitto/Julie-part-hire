...
def display_orders():
    ...
    photo = PhotoImage(file = r"C:\Users\21337\Downloads\images.png")
    photoimage = photo.subsample(6, 6)
    tk.Button(print_order_window, text='Delete Selected!', image=photoimage, compound=tk.LEFT, command=delete_selected).pack(side=tk.TOP)
    tk.Button(print_order_window, text="Back to Menu", command=back_to_menu).pack()

    lift_orders_window()

def hire_return_window(title_text, action_text):
...
ttk.Button(main_frame, text="Hire", command=lambda: hire_return_window("Hire", "Order"), width=20).grid(row=1, column=0, pady=10)
ttk.Button(main_frame, text="Returns/Refund", command=lambda: hire_return_window("Return", "Return"), width=20).grid(row=2, column=0, pady=10)
