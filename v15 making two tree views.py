...
def display_orders():
    ...
    print_order_window.geometry("800x600")
    print_order_window.configure(background="#7bdff2")

    print_order_window.attributes("-top", True)

    columns = ("Receipt Number", "Item", "Item/Return Item", "Number of Items Hired/Returned")

    hire_tree = ttk.Treeview(print_order_window, columns=columns, show="headings", height=10)
    return_tree = ttk.Treeview(print_order_window, columns=columns, show="headings", height=10)
...
def hire_window(title_text, action_text):
    ...
    tk.Button(hire_window_instance, text="Order", command=add_order_wrapper, bg="lightblue").grid(column=1, row=3, padx=10, pady=5, sticky='w')
    tk.Button(hire_window_instance, text="Print Orders", command=display_orders, bg="lightblue").grid(column=1, row=4, padx=10, pady=5, sticky='w')
    tk.Button(hire_window_instance, text="Back to Menu", command=lambda: confirm_back_to_menu(hire_window_instance), bg="lightblue").grid(column=2, row=4, padx=10, pady=5, sticky='w')
...
def return_window(title_text, action_text):
    ...
    enter_receipt_number = tk.Entry(return_window_instance, bg="white")
    ...
    tk.Button(return_window_instance, text="Return", command=add_order_wrapper, bg="lightblue").grid(column=1, row=3, padx=10, pady=5, sticky='w')
    tk.Button(return_window_instance, text="Print Orders", command=display_orders, bg="lightblue").grid(column=1, row=4, padx=10, pady=5, sticky='w')
    tk.Button(return_window_instance, text="Back to Menu", command=lambda: confirm_back_to_menu(return_window_instance), bg="lightblue").grid(column=2, row=4, padx=10, pady=5, sticky='w')
...
main_window.configure(background="#99d19c")

my_label = Label(main_window, image=back)
my_label.place(x=0, y=0, relwidth=1, relheight=1)

welcome = tk.Label(main_window, text="Welcome to Party Hire, Would you like to refund or hire an item?", bg="#7bdff2")
welcome.grid(row=0, column=0, pady=10)

ttk.Button(main_window, text="Hire", command=lambda: hire_window("Hire", "Order"), width=20).grid(row=1, column=0, pady=10)
ttk.Button(main_window, text="Returns/Refund", command=lambda: return_window("Return", "Return"), width=20).grid(row=2, column=0, pady=10)
ttk.Button(main_window, text="Check orders", command=display_orders, width=20).grid(row=3, column=0, pady=10)
ttk.Button(main_window, text="Quit", command=quit_program, width=20).grid(row=4, column=0, pady=10)
